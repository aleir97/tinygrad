...
from tinygrad.features.search import time_linearizer, beam_search, bufs_from_lin, compile_linearizer
...
if __name__ == "__main__":
  ...
  with Context(DEBUG=4):
    ...
    for i, si in enumerate(sched):
      ...
      # try a beam search
      # lin = Linearizer(si.ast, device.linearizer_opts)
      # lin = beam_search(lin, rawbufs, 2, bool(getenv("BEAM_ESTIMATE", 1)))
      # lins.append(lin)

      # benchmark the programs
      choices = []
      for lin in lins:
        compile_linearizer(Device.DEFAULT, lin, print_code=True )
  ...
  #   print(f"******* total {total_tm*1000:.2f} ms, {running_gflops/total_tm:6.0f} GFLOPS, {total_tm*1000/i:6.0f} ms Mean Kernel exec")
